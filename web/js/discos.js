...
var funcionesDisco = {
    ...
    validaModificar: function (event){
        var upc = $("#upc").val();
        var artista = $("#artista").val();
        var album = $("#album").val();
        var genero = $("#genero").val();
        ...
        //el UPC nunca deberia estar vacio en el modificar, porque no lo pueden cambiar, al igual que los ISRC de las canciones
        if(!estaVacio(upc)){
            $('#lblUPC').addClass('lblError').text("(*)UPC: - Debe ingresar un UPC.");
            mensaje = mensaje +"UPC\n";
        }else if(!soloNumero(upc)){
        ...
        if (precio <= 0){
            $('#lblPrecio').addClass('lblError').text("(*)Precio: - El precio no puede ser cero ni negativo.");
            mensaje = mensaje + "precio\n";
        }else {
            $('#lblPrecio').removeClass('lblError').text("Precio:");
        }
...
$(document).ready(function(){
    $("#dAlta").submit(function(e){
        funcionesDisco.validaAlta(e);
    });
     $("#dModificar").submit(function(e){
        funcionesDisco.validaModificar(e);
    });
});
